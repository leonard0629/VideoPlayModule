...
public class VideoDBManager {
	...
	private com.jiuguo.module.db.VideoDBHelper helper;
		...
		try {
			for (VideoLoad videoLoad : videoLoads) {
				database.execSQL(
						...
						new Object[] { videoLoad.getId(), videoLoad.getTitle(),
								videoLoad.getDescribe(),
								videoLoad.getDuration(),
								videoLoad.getPostDate(),
								videoLoad.getPosterId(),
								videoLoad.getPosterName(),
								videoLoad.getImageUrl(),
								videoLoad.getPlayCount(),
								videoLoad.getFavourCount(),
								videoLoad.getBookCount(),
								videoLoad.getCheckId(), videoLoad.getFileUrl(),
								videoLoad.getDownLoadSize(),
								videoLoad.getTotalSize(), videoLoad.isFinish(),
								videoLoad.isStart(),
								videoLoad.getDownLoadPart(),videoLoad.getType(),videoLoad.isNew()});
			}
	...
	public void updateVideo(VideoLoad videoload) throws SQLException {
		try {
			...
			values.put(VIDEO_ID, videoload.getId());
			values.put(VIDEO_TITLE, videoload.getTitle());
			values.put(VIDEO_DESCRIBE, videoload.getDescribe());
			values.put(VIDEO_DURATION, videoload.getDuration());
			values.put(VIDEO_POSTDATE, videoload.getPostDate());
			values.put(VIDEO_POSTERID, videoload.getPosterId());
			values.put(VIDEO_POSTERNAME, videoload.getPosterName());
			values.put(VIDEO_IMAGEURL, videoload.getImageUrl());
			values.put(VIDEO_PLAYCOUNT, videoload.getPlayCount());
			values.put(VIDEO_FAVOURCOUNT, videoload.getFavourCount());
